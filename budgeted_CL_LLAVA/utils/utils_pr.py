...
def weight_pruning(kwargs, configs, name, w, prune_ratio, mask_fixed_params=None):
    """
    ...
    w_grad = None
    if not w.grad is None and kwargs["sp_lmd"]:
    ...
    if (kwargs["sp_admm_sparsity_type"] == "irregular") or (kwargs["sp_admm_sparsity_type"] == "irregular_global"):

        weight_temp = np.abs(weight)  # a buffer that holds weights with absolute values
        if not w_grad is None:
            grad_temp = np.abs(w_grad)
            imp_temp = weight_temp + (kwargs["sp_lmd"] * grad_temp)
        else:
    ...
    elif (kwargs["sp_admm_sparsity_type"] == "random_irregular"):
    ...
    elif (kwargs["sp_admm_sparsity_type"] == "pattern"):
        ...
        ww = weight_ori * non_zeros
        return torch.from_numpy(non_zeros).cuda(), torch.from_numpy(ww).cuda()
    elif (kwargs["sp_admm_sparsity_type"] == "random_pattern"):
        print("pattern pruning...", weight.shape)
        shape = weight.shape
    ...
    elif (kwargs["sp_admm_sparsity_type"] == "filter_balance"):
    ...
    elif (kwargs["sp_admm_sparsity_type"] == "free_block_prune_column_4"):
        shape = weight.shape
        weight2d = weight.reshape(shape[0], -1)
        shape2d = weight2d.shape
    ...
    raise SyntaxError("Unknown sparsity type: {}".format(kwargs["sp_admm_sparsity_type"]))


def weight_growing(kwargs, name, pruned_weight_np, lower_bound_value, upper_bound_value, update_init_method, mask_fixed_params=None):
    shape = None
    weight1d = None
    ...
    if (kwargs["sp_admm_sparsity_type"] == "irregular"):
    ...
    elif "pattern" in kwargs["sp_admm_sparsity_type"]:
