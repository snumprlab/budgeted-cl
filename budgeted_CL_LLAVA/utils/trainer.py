...
import numpy as np
from utils.block_utils import MODEL_BLOCK_DICT, MODEL_TRAINABLE_BLOCK_DICT, get_llava_blockwise_flops
...
def create_trainer(model, tokenizer, training_args, data_module, ours=False):
    if ours:
        print("@@ours@@")
        ...
        print("@@vanila@@")
        trainer = CustomLLaVATrainer_Vanila(model=model,
            tokenizer=tokenizer,
            args=training_args,
            packing=True,
            max_seq_length=training_args.model_max_length,
...
class CustomLLaVATrainer_Vanila(LLaVATrainer):
    ...
    def __init__(
        ...
        target_update_parameters = MODEL_TRAINABLE_BLOCK_DICT['llava']
    ...
    def get_grad(self, logit, label, weight):
        ...
        if label.dim() == y_hat.dim() - 1:
            label = label.unsqueeze(-1)

        with contextlib.nullcontext():
            return torch.matmul((y_hat - label).bfloat16(), weight)
    ...
    def set_flops_dict(self, flops_dict):
        ...
        # Vision encoder를 아예 날려버리기 위해서
        # forward_flops = forward_flops[1:]
        # backward_flops = backward_flops[1:]
        ...
        print("blockwise_backward_flops", len(self.blockwise_backward_flops))
        print(self.blockwise_backward_flops)
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                self.total_flops += (inputs["labels"].shape[0] * (self.forward_flops + self.backward_flops))
                print()
...
class CustomLLaVATrainer(LLaVATrainer):
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if not error and not self.frozen:
            self.calculate_fisher(model)
    ...
    def get_freeze_idx(self, logit, label):
        ...
        last_layer_weights = self.model.base_model.lm_head.weight
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if len(self.freeze_idx) == 0:    
                    ...
                    self.total_flops += (inputs["labels"].shape[0] * (self.forward_flops + self.backward_flops))
                    print("whole", self.backward_flops)
                else:
                    self.total_flops += (inputs["labels"].shape[0] * (self.forward_flops + self.get_backward_flops()))
                    print("not whole", self.freeze_idx)
                    print("self.get_backward_flops()", self.get_backward_flops())
                print("self.total_flops", self.total_flops)
    ...
    def unfreeze_layers(self, model):
        ...
        for name, param in model.named_parameters():
            if ".".join(name.split(".")[1:-1]) in self.target_update_parameters:
    ...
    @torch.no_grad()
    def calculate_fisher(self, model):
        ...
        for i, block_name in enumerate(self.block_names[:-1]):
            for subblock_name in block_name:
                ...
                block = get_attr(model)
                for n, p in block.named_parameters():
                    if p.requires_grad:
                        grad = deepspeed.utils.safe_get_full_grad(p)
                        if grad is not None:
                            # print("yes grad", f"subblock_name.{n}")
                            if not grad.isnan().any():
                                block_fisher[i] += (grad.clone().detach().clamp(-1, 1) ** 2).sum().item()
