...
if __name__ == '__main__':
    ...
    print(f"start evaluating {args.note}")

    client = OpenAI(
        # This is the default and can be omitted
        api_key=os.environ.get(args.key),
    )
    for train_task in range(1, 1+args.num_tasks):
        for eval_task in range(1, task+1):
            json_file_name = f"task{train_task}_evaltask{eval_task}_{args.dataset}.json"
            output_json_file_name = f"task{train_task}_evaltask{eval_task}_{args.dataset}_gpt.json"
            results_dict = json.load(open(os.path.join(args.prefix, args.note, args.seed, json_file_name), 'r'))

            if os.path.isfile(os.path.join(args.prefix, args.note, args.seed, output_json_file_name)):
                cur_reviews = [json.loads(line) for line in open(os.path.join(args.prefix, args.note, args.seed, output_json_file_name))]
            else:
                # output_dir = os.path.dirname(output_json_file_name)
                # os.makedirs(output_dir, exist_ok=True)
                cur_reviews = []

            # if os.path.isfile(os.path.expanduser(output_json_file_name)):
            #     cur_reviews = [json.loads(line) for line in open(os.path.expanduser(output_json_file_name))]
            # else:
            #     output_dir = os.path.dirname(output_json_file_name)
            #     os.makedirs(output_dir, exist_ok=True)
            #     cur_reviews = []

            review_file = open(f'{os.path.join(args.prefix, args.note, args.seed, output_json_file_name)}', 'a')
