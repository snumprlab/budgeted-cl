...
def get_op(
    ...
    if op_name == "ShearX":
        ...
        return RandomAffine(degrees=0, translate=None, scale=None, shear=[-magnitude,magnitude,0,0], p=1.0)

    elif op_name == "ShearY":
        #magnitude = abs(magnitude)
        return RandomAffine(degrees=0, translate=None, scale=None, shear=[0,0,-magnitude,magnitude], p=1.0)

    elif op_name == "TranslateX":
        #return RandomAffine(degrees=0, translate=(150.0 / 331.0, 0), scale=None, shear=None, p=1.0)
        return RandomAffine(degrees=0, translate=(magnitude, 0), scale=None, shear=None, p=1.0)

    elif op_name == "TranslateY":
        #return RandomAffine(degrees=0, translate=(0, 150.0 / 331.0), scale=None, shear=None, p=1.0)
        return RandomAffine(degrees=0, translate=(0, magnitude), scale=None, shear=None, p=1.0)

    elif op_name == "Rotate":
        # img = F.rotate(img, magnitude, interpolation=interpolation, fill=fill)
        #return RandomAffine(degrees=30, translate=None, scale=None, shear=None)
        return RandomAffine(degrees=magnitude, translate=None, scale=None, shear=None)
...
class Kornia_Randaugment(torch.nn.Module):
    ...
    def _augmentation_space(self, num_bins: int) -> Dict[str, Tuple[Tensor, bool]]:
        ...
        return {
            ...
            "Identity": (torch.tensor(0.0), False),
            "ShearX": (self.set_aug_space(0.0, 180.0, num_bins), True),
            "ShearY": (self.set_aug_space(0.0, 180.0, num_bins), True),
            "TranslateX": (self.set_aug_space(0, 150.0 / 331.0, num_bins), True),
            "TranslateY": (self.set_aug_space(0, 150.0 / 331.0, num_bins), True),
            "Rotate": (self.set_aug_space(0, 30, num_bins), True),
        ...
        return {
            "Identity": (torch.tensor(0.0), False),
            "ShearX": (torch.linspace(0.0, 180.0, num_bins), False),
            "ShearY": (torch.linspace(0.0, 180.0, num_bins), False),
            "TranslateX": (torch.linspace(0.0, 150.0 / 331.0, num_bins), False),
            "TranslateY": (torch.linspace(0.0, 150.0 / 331.0, num_bins), False),
            "Rotate": (torch.linspace(0.0, 30.0, num_bins), False),
            "Brightness": (torch.linspace(0.0, 0.9, num_bins), True),
            "Color": (torch.linspace(0.0, 0.9, num_bins), True),
            "Contrast": (torch.linspace(0.0, 0.9, num_bins), True),
            "Sharpness": (torch.linspace(0.0, 0.9, num_bins), True),
    ...
    def form_transforms(self, klasses=None):
        ...
        if klasses is None:
            ...
            for _ in range(self.num_ops):
                ...
                magnitude = float(magnitudes[self.rand_magnitude].item()) if magnitudes.ndim > 0 else 0.0
