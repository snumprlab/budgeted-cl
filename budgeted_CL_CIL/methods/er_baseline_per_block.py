...
class ER:
    ...
    def online_train(self, sample, batch_size, n_worker, sample_num, iterations=1, stream_batch_size=1):
        ...
        for i in range(iterations):
            ...
            cls_acc_dict = {}
            for cls, acc in enumerate(cls_acc):
                if acc != 0.0:
                    cls_acc_dict[str(cls)] = acc

            print("train_cls_acc")
            print(cls_acc)
            print("train_cls_acc_dict")
            print(cls_acc_dict)
            self.writer.add_scalars(f"train/cls_acc", cls_acc_dict, sample_num)
            '''
    ...
    def online_evaluate(self, test_list, sample_num, batch_size, n_worker, cls_dict, cls_addition, data_time):
        ...
        eval_dict = self.evaluation(test_loader, eval_criterion, sample_num)
    ...
    def evaluation(self, test_loader, criterion, sample_num):
        total_correct, total_num_data, total_loss = 0.0, 0.0, 0.0
        correct_l = torch.zeros(self.n_classes)
        num_data_l = torch.zeros(self.n_classes)
        cls_loss = torch.zeros(self.n_classes)
        label = []
        ...
        with torch.no_grad():
            for i, data in enumerate(test_loader):
                ...
                batch_loss = criterion(logit, y)
                loss = torch.mean(batch_loss)
                for y_index, y_value in enumerate(y):
                    cls_loss[y_value] += batch_loss[y_index].item()
        ...
        cls_acc = (correct_l / (num_data_l + 1e-5)).numpy().tolist()
        cls_avg_loss = (cls_loss / (num_data_l + 1e-5)).numpy().tolist()
        ...
        for cls, acc in enumerate(cls_acc):
            if acc != 0.0:
                cls_acc_dict[str(cls)] = acc
            cls_avg_loss_dict[str(cls)] = cls_avg_loss[cls]

        print("test_cls_acc")
        print(cls_acc)
        print("test_cls_acc_dict")
        print(cls_acc_dict)
        print("test_cls_loss_dict")
        print(cls_avg_loss_dict)
        self.writer.add_scalars(f"test/cls_acc", cls_acc_dict, sample_num) 
        self.writer.add_scalars(f"test/cls_loss", cls_avg_loss_dict, sample_num)
